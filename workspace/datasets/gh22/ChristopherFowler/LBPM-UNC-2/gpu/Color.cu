...
__global__  void dvc_ScaLBL_Color_BC_z(int *list, int *Map, double *Phi, double *DenA,double* DenB, double vA, double vB, int count, int Np)
{
	int idx,n,nm;
	// Fill the outlet with component b
	idx = blockIdx.x*blockDim.x + threadIdx.x;
...
__global__  void dvc_ScaLBL_D3Q19_AAeven_Color_with_time_averaged_velocity(int *Map, double *dist, double *Aq, double *Bq, double *Den, double *Phi,
        double *Velocity, double rhoA, double rhoB, double tauA, double tauB, double alpha, double beta,
        double Fx, double Fy, double Fz, int strideY, int strideZ, int start, int finish, int Np, int timestep,double * time_averaged_velocity){
    int ijk,nn,n;
    double fq;
    // conserved momemnts
    double rho,jx,jy,jz;
    ...
    for (int s=0; s<S; s++){
        ...
        if (n<finish) {
            ...
            time_averaged_velocity[n] += ux;
            time_averaged_velocity[Np+n] += uy;
            time_averaged_velocity[2*Np+n] += uz;
...
__global__ void dvc_ScaLBL_D3Q19_AAodd_Color_smooth(int *neighborList, int *Map, double *dist, double *Aq, double *Bq, double *Den,
         double *Phi, double *Velocity, double rhoA, double rhoB, double tauA, double tauB, double alpha, double beta,
        double Fx, double Fy, double Fz, int strideY, int strideZ, int start, int finish, int Np,int timestep){
...
__global__ void dvc_save_scalar(double *old_scalar_field, double * new_scalar_field, int N) {
    int n;
            int S = N/NBLOCKS/NTHREADS + 1;
        for (int s=0; s<S; s++){
            //........Get 1-D index for this thread....................
            n =  S*blockIdx.x*blockDim.x + s*blockDim.x + threadIdx.x + 0;
            if (n<N) {
...
__global__ void dvc_save_state(double *dist, double* saved_dist, int start, int finish, int q, int Np){
	...
	int n; 
        ...
        for (int s=0; s<S; s++){
            ...
            if (n<finish) {
                ...
                saved_dist[n + 0*Np] = dist[n + 0*Np];
                saved_dist[n + 1*Np] = dist[n + 1*Np];
                // Autogenerating LBM block...
                 saved_dist[n + 2*Np] = dist[n + 2*Np];
                 saved_dist[n + 3*Np] = dist[n + 3*Np];
                 saved_dist[n + 4*Np] = dist[n + 4*Np];
                 saved_dist[n + 5*Np] = dist[n + 5*Np];
                 saved_dist[n + 6*Np] = dist[n + 6*Np];
                 saved_dist[n + 7*Np] = dist[n + 7*Np];
                 saved_dist[n + 8*Np] = dist[n + 8*Np];
                 saved_dist[n + 9*Np] = dist[n + 9*Np];
                 saved_dist[n + 10*Np] = dist[n + 10*Np];
                 saved_dist[n + 11*Np] = dist[n + 11*Np];
                 saved_dist[n + 12*Np] = dist[n + 12*Np];
                 saved_dist[n + 13*Np] = dist[n + 13*Np];
                 saved_dist[n + 14*Np] = dist[n + 14*Np];
                 saved_dist[n + 15*Np] = dist[n + 15*Np];
                 saved_dist[n + 16*Np] = dist[n + 16*Np];
                 saved_dist[n + 17*Np] = dist[n + 17*Np];
                 saved_dist[n + 18*Np] = dist[n + 18*Np];    //done...
...
extern "C" void save_state(double *dist, double* saved_dist, int start, int finish, int q, int Np){
    ...
    cudaProfilerStart();
    cudaFuncSetCacheConfig(dvc_save_state, cudaFuncCachePreferL1);
    
    dvc_save_state<<<NBLOCKS,NTHREADS >>>(dist,saved_dist,start,finish,q,Np);
    
    cudaError_t err = cudaGetLastError();
    if (cudaSuccess != err){
        printf("CUDA error in save_state: %s \n",cudaGetErrorString(err));
    }
    cudaProfilerStop();
}
...
extern "C" void save_scalar(double *old_scalar_field, double * new_scalar_field, int N) {
    ...
    cudaProfilerStart();
    cudaFuncSetCacheConfig(dvc_save_scalar, cudaFuncCachePreferL1);
    
    dvc_save_scalar<<<NBLOCKS,NTHREADS >>>(old_scalar_field, new_scalar_field, N);
...
extern "C" void ScaLBL_D3Q19_AAeven_Color_with_time_averaged_velocity(int *Map, double *dist, double *Aq, double *Bq, double *Den, double *Phi,
        double *Vel, double rhoA, double rhoB, double tauA, double tauB, double alpha, double beta,
        double Fx, double Fy, double Fz, int strideY, int strideZ, int start, int finish, int Np,int timestep,double * time_averaged_velocity){
    ...
    dvc_ScaLBL_D3Q19_AAeven_Color_with_time_averaged_velocity<<<NBLOCKS,NTHREADS >>>(Map, dist, Aq, Bq, Den, Phi, Vel, rhoA, rhoB, tauA, tauB,
            alpha, beta, Fx, Fy, Fz, strideY, strideZ, start, finish, Np,timestep,time_averaged_velocity);
    cudaError_t err = cudaGetLastError();
    if (cudaSuccess != err){
        printf("CUDA error in ScaLBL_D3Q19_AAeven_Color: %s \n",cudaGetErrorString(err));
    }
...
extern "C" void ScaLBL_D3Q19_AAodd_Color_smooth(int *d_neighborList, int *Map, double *dist, double *Aq, double *Bq, double *Den,
        double *Phi, double *Vel, double rhoA, double rhoB, double tauA, double tauB, double alpha, double beta,
        double Fx, double Fy, double Fz, int strideY, int strideZ, int start, int finish, int Np,int timestep){
    ...
    dvc_ScaLBL_D3Q19_AAodd_Color_smooth<<<NBLOCKS,NTHREADS >>>(d_neighborList, Map, dist, Aq, Bq, Den, Phi, Vel,
            rhoA, rhoB, tauA, tauB, alpha, beta, Fx, Fy, Fz, strideY, strideZ, start, finish, Np,timestep);
...
extern "C" void ScaLBL_Color_BC_z(int *list, int *Map, double *Phi, double *DenA, double* DenB, double vA, double vB, int count, int Np){
    ...
    dvc_ScaLBL_Color_BC_z<<<GRID,512>>>(list, Map, Phi, DenA,DenB, vA, vB, count, Np);
	cudaError_t err = cudaGetLastError();
	if (cudaSuccess != err){
