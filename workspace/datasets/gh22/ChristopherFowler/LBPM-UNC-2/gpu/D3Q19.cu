...
__global__  void dvc_Accumulate_Momentum(double *tavel, double *vel, int N)
{
        int n;
        // distributions
        double f1,f2,f3,f4,f5,f6,f7,f8,f9;
        ...
        for (int s=0; s<S; s++){
                ...
                if (n<N){
                        ...
                        tavel[n] += vel[n];
                        tavel[N+n] += vel[N+n]; 
                        tavel[2*N+n] += vel[2*N+n];
                        //........................................................................
                }
        }
}

__global__  void dvc_Divide_Accumulated_Momentum(double *tavel, double factor, int N)
{
        int n;
        // distributions
...
__global__  void dvc_Clear_Accumulated_Momentum(double *tavel, int N)
{
        int n;
        // distributions
...
extern "C" void Accumulate_Momentum(double *tavel, double *vel, int Np){
       dvc_Accumulate_Momentum<<<NBLOCKS,NTHREADS >>>(tavel, vel, Np);
        cudaError_t err = cudaGetLastError();
        if (cudaSuccess != err){
...
extern "C" void Clear_Accumulated_Momentum(double *tavel, int Np){
       dvc_Clear_Accumulated_Momentum<<<NBLOCKS,NTHREADS >>>(tavel, Np);
        cudaError_t err = cudaGetLastError();
        if (cudaSuccess != err){
                printf("CUDA error in ScaLBL_D3Q19_Velocity: %s \n",cudaGetErrorString(err));
        }
...
extern "C" void Divide_Accumulated_Momentum(double *tavel, double factor, int Np){
       dvc_Divide_Accumulated_Momentum<<<NBLOCKS,NTHREADS >>>(tavel, factor, Np);
        cudaError_t err = cudaGetLastError();
        if (cudaSuccess != err){
                printf("CUDA error in ScaLBL_D3Q19_Velocity: %s \n",cudaGetErrorString(err));
        }
...
__global__ void dvc_ScaLBL_D3Q19_Pressure_BC_z(int *list, double *dist, double din, int count, int Np)
{
    int n,idx;
    // distributions
    double f0,f1,f2,f3,f4,f5,f6,f7,f8,f9;
...
__global__ void dvc_ScaLBL_D3Q19_Pressure_BC_Z(int *list, double *dist, double dout, int count, int Np)
{
    int n,idx;
    // distributions
    double f0,f1,f2,f3,f4,f5,f6,f7,f8,f9;
...
extern "C" void ScaLBL_D3Q19_Pressure_BC_Z(int *list, double *dist, double dout, int count, int Np){
	int GRID = count / 512 + 1;
	dvc_ScaLBL_D3Q19_Pressure_BC_Z<<<GRID,512>>>(list,dist, dout, count, Np);
}
extern "C" void ScaLBL_D3Q19_Pressure_BC_z(int *list, double *dist, double dout, int count, int Np){
    int GRID = count / 512 + 1;
    dvc_ScaLBL_D3Q19_Pressure_BC_Z<<<GRID,512>>>(list,dist, dout, count, Np);
