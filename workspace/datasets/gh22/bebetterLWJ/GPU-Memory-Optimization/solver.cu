...
float Solver::step(int start_X, int start_y, std::vector<float>& fwd_ldnn_lag, std::vector<float>& bwd_ldnn_lag) {
	float temp_loss;
	 std::cout << "start_X: " << start_X << std::endl;
	 std::cout << "start_Y " << start_y << std::endl;
	if (model->data_type == CUDNN_DATA_FLOAT)
		model->getLoss(&(((float*)X_train)[start_X]), &y_train[start_y], learning_rate, fwd_ldnn_lag, bwd_ldnn_lag, true, NULL, &temp_loss);
	else if (model->data_type == CUDNN_DATA_DOUBLE)
		model->getLoss(&(((double*)X_train)[start_X]), &y_train[start_y], learning_rate, fwd_ldnn_lag, bwd_ldnn_lag, true, NULL, &temp_loss);

	// float Salpha = -learning_rate;注释
...
void Solver::train(std::vector<float>& loss, std::vector<int>& val_acc) {
	...
	for (int i = 0; i < num_epoch; i++) {
		...
		// learning_rate *= learning_rate_decay;注释
...
void Solver::getTrainTime(std::vector<float>& loss, std::vector<float>& time, int num_epoch,
	std::vector<std::vector<float> >& fwd_ldnn_lag, std::vector<std::vector<float> >& bwd_ldnn_lag) {
	int batch_size = model->batch_size;
	int num_train_batches = num_train / model->batch_size;
	for (int i = 0; i < num_epoch; i++) {
		for (int j = 0; j < num_train_batches; j++) {
			...
			std::vector<float> cur_fwd_ldnn_lag, cur_bwd_ldnn_lag;
			float temp_loss = step(start_sample, j * batch_size, cur_fwd_ldnn_lag, cur_bwd_ldnn_lag); //出错的函数调用
			...
			fwd_ldnn_lag.push_back(cur_fwd_ldnn_lag); //看到push_back就知道是集合
			bwd_ldnn_lag.push_back(cur_bwd_ldnn_lag);
