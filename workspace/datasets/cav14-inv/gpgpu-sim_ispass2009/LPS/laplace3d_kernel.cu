...
#define KOFF (BLOCK_X+2)*(BLOCK_Y+2)
  ...
  i = (threadIdx.y<2) ? threadIdx.x : (k%2)*(BLOCK_X+1) - 1;
  j = (threadIdx.y<2) ? threadIdx.y*(BLOCK_Y+1) - 1 : k/2 - BLOCK_X - 1;
  ...
  for (k=0; k<NZ; k++) {
    int indg0 = indg + k * (NY * pitch);
    int indg_ = indg + (k + 1) * (NY * pitch);
    int indg_h_ = indg_h + (k + 1) * (NY * pitch);
