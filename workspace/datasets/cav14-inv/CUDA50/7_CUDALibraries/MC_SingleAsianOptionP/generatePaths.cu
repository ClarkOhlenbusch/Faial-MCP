                              ...
                              const unsigned int numTimesteps)
{
    ...
    for (unsigned int i = tid ;
         ...
         __global_invariant(__write_implies(paths, (__write_offset(paths)/sizeof(Real))%numSims%step == tid)),
         i < numSims ; i += step)
    {
        ...
        for (unsigned int t = 0 ;
             __invariant(i%step == tid),
             __invariant(__ptr_offset(output)/sizeof(Real)%numSims%step == tid),
             __global_invariant(__write_implies(paths, (__write_offset(paths)/sizeof(Real))%numSims%step == tid)),
