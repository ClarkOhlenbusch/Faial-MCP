...
)
{
    ...
    for (int opt = tid;
        ...
        __global_invariant(__write_implies(d_CallResult, __write_offset(d_CallResult)/sizeof(float) % THREAD_N == tid)),
        __global_invariant(__write_implies(d_PutResult, __write_offset(d_PutResult)/sizeof(float) % THREAD_N == tid)),
