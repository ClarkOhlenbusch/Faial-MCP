               ...
               int w, int h, float fscale)
{
    ...
    for (int i = threadIdx.x;
         ...
         __global_invariant(__write_implies(pSobelOriginal, __write_offset(pSobelOriginal)/Pitch == blockIdx.x)),
         __global_invariant(__write_implies(pSobelOriginal, __write_offset(pSobelOriginal)%Pitch%blockDim.x == threadIdx.x)),
         i < w; i += blockDim.x)
    {
