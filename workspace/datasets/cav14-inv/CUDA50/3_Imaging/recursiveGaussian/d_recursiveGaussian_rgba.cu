...
d_recursiveGaussian_rgba(uint *id, uint *od, int w, int h, float a0, float a1, float a2, float a3, float b1, float b2, float coefp, float coefn)
{
    ...
    for (int y = 0;
         __global_invariant(__implies(x >= w, !__enabled())),
         __global_invariant(__implies(x < w, (__ptr_offset(od)/sizeof(uint))%w == x)),
         __global_invariant(__write_implies(od, (__write_offset(od)/sizeof(uint))%w == x)),
         y < h; y++)
    {
    ...
    for (int y = h-1;
         ...
         __global_invariant(__implies(x < w, (__ptr_offset(od)/sizeof(uint))%w == x)),
         __global_invariant(__write_implies(od, (__write_offset(od)/sizeof(uint))%w == x)),
         __global_invariant(__read_implies(od, (__read_offset(od)/sizeof(uint))%w == x)),
         y >= 0; y--)
    {
        float4 xc = rgbaIntToFloat(*id);
