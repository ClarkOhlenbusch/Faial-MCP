...
#pragma unroll
    ...
    for (uint pos = UMAD(blockIdx.x, blockDim.x, threadIdx.x);
         __global_invariant(__write_implies(s_Hist, (__write_offset(s_Hist)/sizeof(uchar) - threadPos)%HISTOGRAM64_THREADBLOCK_SIZE == 0)),
         __global_invariant(__read_implies(s_Hist, (__read_offset(s_Hist)/sizeof(uchar) - threadPos)%HISTOGRAM64_THREADBLOCK_SIZE == 0)),
         pos < dataCount; pos += UMUL(blockDim.x, gridDim.x))
    {
