...
{
    __requires(imageH == 320);
    __requires(imageW == 408);
    //Weights cache
    ...
    if (ix < imageW && iy < imageH)
    {
        ...
        // __syncthreads(); // DATA RACE: Missing syncthreads allows threads to read weights from shared memory before all threads finish computing and writing them
