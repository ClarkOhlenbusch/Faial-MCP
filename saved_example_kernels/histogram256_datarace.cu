...
#pragma unroll
    ...
    for (uint i = 0;
         __invariant(i <= (HISTOGRAM256_THREADBLOCK_MEMORY / HISTOGRAM256_THREADBLOCK_SIZE)),
         __invariant(__write_implies(s_Hist, ((__write_offset(s_Hist)/sizeof(uint))%HISTOGRAM256_THREADBLOCK_SIZE) == threadIdx.x)),
             i < (HISTOGRAM256_THREADBLOCK_MEMORY / HISTOGRAM256_THREADBLOCK_SIZE); i++)
    {
    ...
    for (uint bin = threadIdx.x;
         ...
         __global_invariant(__write_implies(d_PartialHistograms, __write_offset(d_PartialHistograms)/sizeof(uint) % HISTOGRAM256_BIN_COUNT % blockDim.x == threadIdx.x)),
         bin < HISTOGRAM256_BIN_COUNT; bin += HISTOGRAM256_THREADBLOCK_SIZE)
    {
        uint sum = 0;
