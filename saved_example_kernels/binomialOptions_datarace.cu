...
__global__ void binomialOptionsKernel(
  ...
  real *d_CallBuffer           // imperial edit
    ...
    for (int i = tid;
      // JPW: Each thread writes different indices of d_Call:
      __global_invariant((unsigned)i % CACHE_SIZE == tid),
      // JPW: Each block writes only within its segment of d_CallBuffer
      __global_invariant(__write_implies(d_CallBuffer, (__write_offset(d_CallBuffer)/sizeof(real))/(NUM_STEPS + 16) == blockIdx.x)),
    ...
    for (int i = NUM_STEPS;
    // JPW: The following invariant is not inferred, but it should
    //      be (by the initial assignment to i and the fact that i
    //      only decreases)
    __global_invariant(i <= NUM_STEPS),
    // JPW: Each block reads only within its segment of d_CallBuffer
    __global_invariant(__read_implies(d_CallBuffer, (__read_offset(d_CallBuffer)/sizeof(real))/(NUM_STEPS + 16) == blockIdx.x)),
    // JPW: Each block writes only within its segment of d_CallBuffer
    __global_invariant(__write_implies(d_CallBuffer, (__write_offset(d_CallBuffer)/sizeof(real))/(NUM_STEPS + 16) == blockIdx.x)),
    i > 0; i -= CACHE_DELTA)
        for (int c_base = 0;
        ...
        c_base < i; c_base += CACHE_STEP)
        {
            ...
            // __syncthreads(); // DATA RACE: Missing syncthreads allows threads to read from shared memory before previous iteration writes are visible
