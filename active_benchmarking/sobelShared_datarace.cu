            ...
            short w, short h, float fScale)
{
    ...
    for (ib = threadIdx.x;
         ...
         __global_invariant(__write_implies(LocalBlock, (__write_offset(LocalBlock)-SharedIdx) >= 0)),
         __global_invariant(__write_implies(LocalBlock, (__write_offset(LocalBlock)-SharedIdx)/4 < (BlockWidth+2*RADIUS))),
         __global_invariant(__write_implies(LocalBlock, (__write_offset(LocalBlock)-SharedIdx)/4%blockDim.x == threadIdx.x)),
    ...
    if (threadIdx.y < RADIUS*2)
    {
        ...
        for (ib = threadIdx.x;
             ...
             __global_invariant(__implies(threadIdx.y < RADIUS*2, __write_implies(LocalBlock,
                  (((__write_offset(LocalBlock)-SharedIdx) >= 0)
                   & ((__write_offset(LocalBlock)-SharedIdx)/4 < (BlockWidth+2*RADIUS))
                   & ((__write_offset(LocalBlock)-SharedIdx)/4%blockDim.x == threadIdx.x))
                | (((__write_offset(LocalBlock)-SharedIdxOld) >= 0)
                   & ((__write_offset(LocalBlock)-SharedIdxOld)/4 < (BlockWidth+2*RADIUS))
                   & ((__write_offset(LocalBlock)-SharedIdxOld)/4%blockDim.x == threadIdx.x))))),
             __global_invariant(__implies(threadIdx.y >= RADIUS*2, __write_implies(LocalBlock,
                  (((__write_offset(LocalBlock)-SharedIdxOld) >= 0)
                   & ((__write_offset(LocalBlock)-SharedIdxOld)/4 < (BlockWidth+2*RADIUS))
                   & ((__write_offset(LocalBlock)-SharedIdxOld)/4%blockDim.x == threadIdx.x))))),
    ...
    for (ib = threadIdx.x;
         __global_invariant(ib%blockDim.x == threadIdx.x),
         __global_invariant(__write_implies(pSobelOriginal, (((__write_offset(pSobelOriginal) - v*SobelPitch)/sizeof(uchar4)) - u)%blockDim.x == threadIdx.x)),
         __global_invariant(__write_implies(pSobelOriginal, (((__write_offset(pSobelOriginal) - v*SobelPitch)/sizeof(uchar4)) - u) < BlockWidth)),
         __global_invariant(__write_implies(pSobelOriginal, (__write_offset(pSobelOriginal) < (v + 1)*SobelPitch))),
