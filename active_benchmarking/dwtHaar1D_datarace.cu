          ...
          const int bdim)
{
    ...
    if (dlevels > 1)
    {
        ...
        for (unsigned int i = 1;
              __invariant(__implies(tid < num_threads, num_threads == bdim >> i)),
              __invariant(__implies(tid < num_threads, offset_neighbor == 1 << (i - 1))),
              __invariant(__implies(tid < num_threads, idata0 == tid << i)),
              __invariant(i >= 1),
              i < dlevels; ++i)
        {
